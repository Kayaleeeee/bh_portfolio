...
export const VIMEO_AUTH_TOKEN = `2c3e2418f9bb7808fcfe3b393a64b8ee`;
...
export const getVimeoDataList = (searchParams: {
  ...
  return axios
    .create({
      headers: { Authorization: `bearer ${VIMEO_AUTH_TOKEN}` },
    })
    .get(
      `${VIMEO_BASE_URL}/channels/${VIMEO_CHANNEL_ID}/videos?per_page=${searchParams.per_page}&page=${searchParams.page}&sort=manual&sizes=480`
    );
};
